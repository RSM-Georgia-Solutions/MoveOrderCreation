...
namespace MoveOrdersCreation
{
    public class DiManager
    {
        ...
        public bool CreateTable(string tableName, BoUTBTableType TableType)
        {
            ...
            try
            {
                ...
                if (oUTables.GetByKey(tableName) == false)
                {
                    ...
                    {
                        SAPbouiCOM.Framework.Application.SBO_Application.StatusBar.SetText("UDT failed: " + Company.GetLastErrorDescription(), SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error);
                        System.Runtime.InteropServices.Marshal.ReleaseComObject(oUTables);
                        GC.Collect();
                        return false;
                    }
        ...
        private void AddUDORecord2()
        {
            int i = 1;
            try
            {
                CompanyService oCompanyService = DiManager.Company.GetCompanyService();
                // Get GeneralService (oCmpSrv is the CompanyService)
                GeneralService oGeneralService = oCompanyService.GetGeneralService("UDOID");
                // Create data for new row in main UDO
                GeneralData oGeneralData = (GeneralData)oGeneralService.GetDataInterface(GeneralServiceDataInterfaces.gsGeneralData);
                oGeneralData.SetProperty("U_TestField", "test2");
                //  Handle child rows
                GeneralDataCollection oChildren = oGeneralData.Child("UDOROW");
                // Create data for rows in the child table
                GeneralData oChild = oChildren.Add();
                oChild.SetProperty("U_TestFieldRow", "Test2Row");
